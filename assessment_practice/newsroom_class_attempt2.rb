	...
	class Newsroom
		...
		attr_reader :name, :reporters, :overhead, :matches
		...
		def add_reporters(reporter, skills)
			...
			if self.has_budget?(reporter)
				@reporters.store(reporter, skills)
			else
		...
		# Release 8: linking stories with the reporter
		def find_reporters_with_skill(skill)
			@reporters.each{|key,value|
	...
	the_news.add_reporters("Rebecca Berg", ["reporting", "talking politics", "polling"])
	...
	the_news.reporters.each{ |key, value| 
		puts "- #{key}, specializing in #{value.flatten.join(", ")}."
	}
	puts "Thank you for watching! Good night!"

	the_news.find_reporters_with_skill("not aging")
	p the_news.matches